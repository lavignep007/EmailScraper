...
public sealed class DatabaseMessageIdentityTests
{
    ...
    [Fact]
    public async Task Gmail_id_is_preserved_exactly()
    {
        ...
        var stored = await Database.GetMessageByGmailIdAsync(database.Path, gmailId);

        stored.Should().NotBeNull();
        stored!.GmailId.Should().Be(gmailId);
    }

    [Fact]
    public async Task Duplicate_Gmail_id_is_rejected()
    {
        await using var database = await TestDatabase.CreateAsync();
        var gmailId = fixture.Create<string>();
