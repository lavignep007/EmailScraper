...
public static class PdfGenerator
{
        ...
        string archivePath)
    {
        ...
        var threads =
            await Database.GetAllThreadsAsync(
                databasePath);

        Console.WriteLine(
            $"Threads found: {threads.Count:N0}");
        ...
        Console.WriteLine(
            $"Thread PDFs:   {threadCount:N0}");

        Console.WriteLine(
            $"Message PDFs:  {messageCount:N0}");
        ...
        PdfMessage message)
    {
                ...
                message.RelativePath))
        {
            throw new InvalidOperationException(
                $"Message {message.Id} has no RelativePath.");
        }
        ...
        var mime =
            await Task.Run(
                () => MimeMessage.Load(emlPath));

        var attachments =
            await Database
                .GetAttachmentsForMessageAsync(
                    databasePath,
                    message.Id);
        ...
        var fileName =
            $"{message.Id:D6} - " +
            SanitizeFileName(
                ShortenFileNamePart(message.DisplayName ??
                $"Message {message.Id}") +
            ".pdf");
    ...
    {
        var fileName =
            ShortenFileNamePart($"{thread.Id:D6} - " +
            SanitizeFileName(
        ...
        List<PdfMessage> messages)
    {
        container
            .Column(column =>
            {
                ...
                column.Item()
                    .Text(
                        $"First: {thread.FirstDate}");

                column.Item()
                    .Text(
                        $"Last: {thread.LastDate}");
        ...
        string value)
    {
        ...
        value = new string(
            value.Where(c => !char.IsControl(c)).ToArray());
        ...
        long? messageId = null)
    {
        ...
        {
            Console.WriteLine(
                $"Message {messageId}: removing glyph U+F0D8");
        }
