...
const string ApplicationName = "Email Scraper";
...
var lastHistoryId = await Database.GetSyncStateAsync(config.DatabasePath, "LastHistoryId");
...
if (string.IsNullOrWhiteSpace(lastHistoryId))
{
    ...
    await FullSyncAsync(gmail, config);
}
else
{
    Console.WriteLine($"Last history ID: {lastHistoryId}");
    Console.WriteLine();
    ...
    if (input == "F")
        ...
        await IncrementalSyncAsync(gmail, config, lastHistoryId);
}
...
switch (postSyncChoice)
{
    ...
    case "R":
        await RepairCorruptEmlFilesAsync(gmail, config);
    ...
    case "C":
        var gmailQuery = BuildGmailQuery(config.EmailAddresses);

        await ArchiveValidator.ValidateAsync(gmail, config.DatabasePath, gmailQuery);
...
static async Task<GmailService> CreateGmailServiceAsync()
{
    ...
    if (!File.Exists(credentialsPath)) throw new FileNotFoundException($"credentials.json was not found at '{credentialsPath}'.", credentialsPath);
    ...
    AppConfig config)
{
    ...
    var query = BuildGmailQuery(config.EmailAddresses);
    ...
    do
    {
        ...
        foreach (var messageRef in response.Messages)
        {
            ...
            try
            {
                ...
                var imported = await DownloadMessageAsync(gmail, config, messageRef.Id);
    ...
    if (startingHistoryId.HasValue)
        await Database.SetSyncStateAsync(config.DatabasePath, "LastHistoryId", startingHistoryId.Value.ToString());

    await Database.SetSyncStateAsync(config.DatabasePath, "LastFullSyncUtc", DateTimeOffset.UtcNow.ToString("O"));
    ...
    string startHistoryId)
{
    ...
    Console.WriteLine($"Checking changes after history ID {startHistoryId}...");
    ...
    if (messageIds.Count == 0)
    {
        ...
        if (!string.IsNullOrWhiteSpace(newestHistoryId))
            await Database.SetSyncStateAsync(config.DatabasePath, "LastHistoryId", newestHistoryId);

        await Database.SetSyncStateAsync(config.DatabasePath, "LastIncrementalSyncUtc", DateTimeOffset.UtcNow.ToString("O"));
    ...
    foreach (var messageId in messageIds)
    {
        try
        {
            ...
            var imported = await DownloadMessageAsync(gmail, config, messageId);
    ...
    string gmailMessageId)
{
    ...
    if (string.IsNullOrWhiteSpace(message.Raw)) throw new InvalidOperationException($"Message {gmailMessageId} contained no RAW data.");
    ...
    byte[] raw)
{
    ...
    return text.Contains("text/vnd.google.email-reaction+json", StringComparison.OrdinalIgnoreCase);
}
    ...
    byte[] raw)
{
    ...
    foreach (var rawLine in headerText.Split('\n'))
    {
        ...
        if (line.StartsWith(" ") ||
            line.StartsWith("\t"))
        {
            if (currentName != null) headers[currentName] += " " + line.Trim();
...
static int TryGetYear(
    string? date)
{
    if (DateTimeOffset.TryParse(date, out var parsed)) return parsed.Year;
...
static string TryFormatDate(
    string? date)
{
    if (DateTimeOffset.TryParse(date, out var parsed)) return parsed.ToLocalTime().ToString("yyyy-MM-dd_HHmmss");
    ...
    AppConfig config)
{
    ...
    foreach (var item in corrupt)
    {
        try
        {
            ...
            var request = gmail.Users.Messages.Get("me", item.GmailId);
            ...
            if (string.IsNullOrWhiteSpace(gmailMessage.Raw)) throw new InvalidOperationException("Gmail returned no RAW data.");

            var rawBytes = DecodeBase64Url(gmailMessage.Raw);

            if (rawBytes.Length == 0) throw new InvalidOperationException("Decoded Gmail RAW data is empty.");
             ...
             */
            if (string.IsNullOrWhiteSpace(item.FilePath)) throw new InvalidOperationException("Database FilePath is empty.");
            ...
            if (tempInfo.Length == 0) throw new InvalidOperationException("Temporary repaired EML is empty.");
