public static class ThreadBuilder
{
        ...
        string databasePath)
    {
        ...
        Console.WriteLine(
            $"Messages found: {messages.Count:N0}");
         ...
         */
        await Database.ClearThreadsAsync(
            databasePath);
         ...
         * Statistics.
         */
        var messagesWithMessageId =
            messages.Count(x =>
                !string.IsNullOrWhiteSpace(
                    x.MessageId));

        var messagesWithInReplyTo =
            messages.Count(x =>
                !string.IsNullOrWhiteSpace(
                    x.InReplyTo));

        var messagesWithReferences =
            messages.Count(x =>
                !string.IsNullOrWhiteSpace(
                    x.References));

        Console.WriteLine(
            $"Messages with Message-ID : " +
            $"{messagesWithMessageId:N0}");

        Console.WriteLine(
            $"Messages with In-Reply-To: " +
            $"{messagesWithInReplyTo:N0}");

        Console.WriteLine(
            $"Messages with References : " +
            $"{messagesWithReferences:N0}");
         ...
         */
        var standaloneMessages =
            threadGroups.Count(x =>
                x.Count == 1 &&
                !HasThreadRelationship(
                    x[0],
                    byMessageId));

        Console.WriteLine(
            $"Logical thread groups: " +
            $"{threadGroups.Count:N0}");

        Console.WriteLine(
            $"Standalone messages   : " +
            $"{standaloneMessages:N0}");
        ...
        Console.WriteLine(
            $"Threads created: {threadCount:N0}");
        ...
        Dictionary<string, ThreadMessage> byMessageId)
    {
        ...
        var threadId =
            await Database.InsertThreadAsync(
                databasePath,
                new ThreadRecord
                {
                    ThreadKey =
        ...
        Dictionary<string, ThreadMessage> byMessageId)
    {
                ...
                message.InReplyTo))
        {
            ...
            if (byMessageId.ContainsKey(key))
            {
                return true;
            }
        }
    ...
    private sealed class UnionFind
    {
        ...
        private readonly Dictionary<long, int> _rank;
